...
case 'reg':
  ...
  if(isset($_SESSION['reg_submit']) && $_SESSION['reg_submit']>time()-600){
    ...
    if (!empty($tgid)) {
      $row=$DB->query("select * from pay_user where id='$tgid' limit 1")->fetch();
      if (!$row) {
  ...
  if($conf['is_payreg']==1){
    ...
    $data = $type.'|'.$account.'|'.$username.'|'.$url.'|'.$email.'|'.$tgid_n.'|'.$clientip;
    ...
    if($sds){
      ...
      if(!empty($tgid_n)){
        $sj=$conf['tgyj'];
        $abc=$DB->exec("UPDATE `pay_user` SET `money`=money+'{$sj}' WHERE `id`='$tgid_n'");
        $abc1=$DB->exec("UPDATE `pay_user` SET `tgrs`=tgrs+'1' WHERE `id`='$tgid_n'");
      }
      //推广逻辑结束
