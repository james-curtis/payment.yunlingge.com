...
if($verify_result) {//验证成功
  ...
  $trade_status = $_GET['trade_status'];
    ...
    if($srow['status']==0){
      ...
      $key = random(32);
      $sds=$DB->exec("INSERT INTO `pay_user` (`key`, `account`, `username`, `money`, `url`, `email`, `phone`, `settle_id`, `addtime`, `type`, `active`) VALUES ('{$key}', '{$account}', '{$username}', '0', '{$url}', '{$email}', '{$phone}', '{$type}', '{$date}', '1', '1')");
      $pid=$DB->lastInsertId();
      if($sds){
        //云凌支付系统推广逻辑
        if(!empty($tgid)){
