...
case 'edit_info':
 ...
 $key=daddslashes(strip_tags($_POST['key']));
	...
	if($qq==null || $url==null &&$key==null){
		exit('{"code":-1,"msg":"请确保每项都不为空"}');
	}
if(strlen($key)<6){
	exit('{"code":-1,"msg":"对不起，您的密匙小于6位"}');
}
	if($conf['verifytype']==1){
		$sqs=$DB->exec("update `pay_user` set `email` ='{$email}',`qq` ='{$qq}',`url` ='{$url}',`key` ='{$key}' where `id`='$pid'");
	}else{
		$sqs=$DB->exec("update `pay_user` set `qq` ='{$qq}',`url` ='{$url}',`key` ='{$key}' where `id`='$pid'");
	}
	if($sqs || $DB->errorCode()=='0000'){
		exit('{"code":1,"msg":"succ"}');
	}else{
